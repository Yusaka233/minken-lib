...
import { Menu, Layout, Header, Slider, Content, Footer } from 'MINKEN';
...
export default function MenuDemos1() {
  ...
  return (
    <Layout>
      <Header style={{ background: '#0d6efd', fontSize: 16, color: '#fff' }}>
        header
      </Header>
      <Layout>
        <Slider>
          <Menu items={items} width="100%" defaultOpen onSelect={onSelect} />
        </Slider>
        <Content style={{ background: '#20c997', fontSize: 16 }}>
          content
        </Content>
      </Layout>
      <Footer style={{ background: '#0d6efd', fontSize: 16, color: '#fff' }}>
        footer
      </Footer>
    </Layout>
  );
